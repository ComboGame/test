...
export function Title({ content }) {
    return (
        <div className="grid items-center justify-start lg:justify-between gap-4 lg:grid-flow-col w-full">
            <h1 className="font-sans text-white text-3xl font-bold tracking-tight">
                {content}
            </h1>
        </div>
    );
...
export function H2({ content, id }) {
    return (
        <h2 id={id} className="scroll-mt-40 text-2xl font-bold lg:scroll-mt-24 not-prose group lg:-ml-6 lg:grid lg:grid-cols-[auto_1fr] lg:items-center lg:justify-start lg:gap-2">
...
export function CustomImage({ src, width, height }) {
    return (
        <figure className="grid rounded-xl">
